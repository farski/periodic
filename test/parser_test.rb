...
class ParserTest < Test::Unit::TestCase
	context "The parser" do
		setup	do
			@units = Hash.new
			@units[:seconds] = { :value => 1, :labels => %w{ s sec second } }
			@units[:minutes] = { :value => 60, :labels => %w{ m min minute } }
			@units[:hours] = { :value => 3600, :labels => %w{ h hr hour } }
			@units[:days] = { :value => 3600*24, :labels => %w{ d day } }
			@units[:weeks] = { :value => 3600*24*7, :labels => %w{ w week } }
			@units[:months] = { :value => 3600*24*30, :labels => %w{ month } }
			@units[:years] = { :value => 3600*24*365.25, :labels => %w{ y yr year } }
			@units[:decades] = { :value => 3600*24*365.25*10, :labels => %w{ dec decade } }
			@units[:centuries] = { :value => 3600*24*365.25*100, :labels => %w{ c cn cent century centuries } }
			@units[:millennia] = { :value => 3600*24*365.25*1000, :labels => %w{ mil millennium millennia } }
		end
